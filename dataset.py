...
class TextDataset(Dataset):
    ...
    def __init__(self, data_file: str, train: bool = True, sp_model_prefix: str = None,
        ...
        self.sp_model_prefix = sp_model_prefix
        ...
        train_texts = texts[:int(len(texts) * (1 - self.VAL_RATIO))]
        val_texts = texts[-int(len(texts) * self.VAL_RATIO)-1:]
    ...
    def __getitem__(self, item: int) -> Tuple[torch.Tensor, int]:
        ...
        indices = torch.full(size=(self.max_length, ), fill_value=self.pad_id)
        length = len(self.indices[item])
        """
        YOUR CODE HERE (⊃｡•́‿•̀｡)⊃━✿✿✿✿✿✿
        ...
        indices[0] = torch.tensor(self.bos_id)
        indices[self.max_length - 1] = torch.tensor(self.eos_id)
        split_len = self.max_length - 2 if self.max_length - 2 < length else length
        indices[1: split_len + 1] = torch.tensor(self.indices[item])[:split_len]

        return indices, split_len + 2
